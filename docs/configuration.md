...
| `feed_list_query_compute_unread`  | boolean               | If `true`, computes the amount of unread items in a query; note: this may cause a slowdown if you have many queries (set `false` to not compute the unread count)  |
